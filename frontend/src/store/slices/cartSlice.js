...
export const addToCart = createAsyncThunk('cart/add', async ({ itemId, quantity=1, token }, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/cart/add', { method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body:JSON.stringify({itemId,quantity}) });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;
...
export const updateCartItem = createAsyncThunk('cart/update', async ({ itemId, quantity, token }, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/cart/update', { method:'PUT', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body:JSON.stringify({itemId,quantity}) });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;

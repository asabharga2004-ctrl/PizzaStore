...
export const updateOrderStatus = createAsyncThunk('orders/updateStatus', async ({ id, orderStatus, messageText, token }, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/orders/'+id+'/status', { method:'PUT', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body:JSON.stringify({orderStatus,messageText}) });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;
...
export const fetchRevenue = createAsyncThunk('orders/revenue', async (token, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/orders/admin/revenue', { headers:{'Authorization':'Bearer '+token} });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;

...
export const createMenuItem = createAsyncThunk('menu/create', async ({ formData, token }, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/menu', { method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body:JSON.stringify(formData) });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;
...
export const updateMenuItem = createAsyncThunk('menu/update', async ({ id, formData, token }, { rejectWithValue }) => {
  try {
    const res  = await fetch('/api/menu/'+id, { method:'PUT', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body:JSON.stringify(formData) });
    const data = await res.json();
    if (!data.success) return rejectWithValue(data.message);
    return data.data;
